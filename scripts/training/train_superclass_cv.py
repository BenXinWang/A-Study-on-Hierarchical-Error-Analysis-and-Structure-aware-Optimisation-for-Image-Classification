#!/usr/bin/env python3
"""
Cross-validation training script for CIFAR-100 superclass classifier.
...
def run_cv_training(n_folds=5, **kwargs):
    ...
    for fold in range(n_folds):
        ...
        cmd = [
            sys.executable, 'scripts/training/train_superclass.py',
            '--use_cv',
            '--cv_fold', str(fold),
        ...
        try:
            ...
            log_file = Path('checkpoints/logs') / f'superclass_cv_fold_{fold}_training_log_{timestamp}.txt'
            log_file.parent.mkdir(exist_ok=True)
            
            # Run training and capture output to log file
            with open(log_file, 'w') as f:
                f.write(f"Superclass cross-validation fold {fold + 1}/{n_folds} training log\n")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = results_dir / f'superclass_cv_training_results_{timestamp}.json'
    
    with open(results_file, 'w') as f:
        json.dump(cv_summary, f, indent=2)
    
    # Save training log with CV summary
    training_log_file = results_dir / 'superclass_training_log_partial_freeze.txt'
    with open(training_log_file, 'w') as f:
        f.write("Superclass Cross-Validation Training Summary (Partial-Freeze)\n")
        f.write("="*60 + "\n")
        f.write(f"Timestamp: {timestamp}\n")
        ...
        if successful_results:
            ...
            f.write(f"Best model saved to: checkpoints/cv_fold_{best_fold}/best_models/superclass_best.pth\n")
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("SUPERCLASS CROSS-VALIDATION TRAINING SUMMARY")
    print(f"{'='*60}")
    ...
    if successful_results:
        ...
        print(f"Best model saved to: checkpoints/cv_fold_{best_fold}/best_models/superclass_best.pth")
        ...
        current_best_path = 'checkpoints/best_models/superclass_best.pth'
        ...
        if Path(current_best_path).exists():
            try:
                ...
                if best_acc/100 > current_acc:
                    ...
                    import shutil
                    cv_best_path = f'checkpoints/cv_fold_{best_fold}/best_models/superclass_best.pth'
        ...
        best_model_cmd = [
            ...
            '--checkpoint_path', f'checkpoints/cv_fold_{best_fold}/best_models/superclass_best.pth',
            '--save_dir', 'results/superclass_test_results'
        ]
...
def main():
    parser = argparse.ArgumentParser(description='Cross-validation training for CIFAR-100 superclass classifier')
